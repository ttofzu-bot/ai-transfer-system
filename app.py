...
# ---------------------------------------------------------------------------
# QUERY INTELLIGENCE FOR GOOGLE PATENTS
# ---------------------------------------------------------------------------
QUERY_STOPWORDS = {
    "and", "or", "not", "the", "a", "an", "of", "for", "in", "on", "with", "without", "to", "from",
    "using", "based", "method", "methods", "system", "systems", "device", "devices", "technology",
    "tac", "ti", "ab", "cl", "cpc", "ipc", "patent", "patents", "google", "publication",
}
...
def split_prompt_terms(value, max_terms=12):
    if not value or value == "—":
        return []
    raw = re.split(r"[,;\n]+", str(value))
    terms = []
...
def extract_terms_from_query(query, max_terms=8):
    """Extract usable broad terms from strict AI queries."""
    q = normalize_query_string(query)
    q = re.sub(r"\b(TAC|TI|AB|CL|CPC|IPC)\s*=\s*", " ", q, flags=re.I)
    q = re.sub(r"[()=:+/*\\]", " ", q)
    quoted = re.findall(r'"([^"]{3,70})"', q)
    q_no_quotes = re.sub(r'"[^"]+"', " ", q)
    words = re.findall(r"[A-Za-z][A-Za-z0-9\-]{2,}", q_no_quotes)
    terms = []
    for phrase in quoted:
        phrase = phrase.strip()
        if phrase.lower() not in QUERY_STOPWORDS:
            terms.append(phrase)
    for w in words:
        wl = w.lower()
        if wl not in QUERY_STOPWORDS and len(w) > 2:
            terms.append(w)
    return dedupe_keep_order(terms)[:max_terms]


def gp_term(term, exact=False):
    """Format one term for Google Patents. Exact phrases are useful, but too many quotes can over-restrict."""
    term = normalize_query_string(term).strip("()")
    if not term:
        return ""
    if exact and " " in term and '"' not in term:
        return f'"{term}"'
    return term



def quote_gp_phrase(term):
    """Quote multi-word phrases safely for Google Patents."""
    term = normalize_query_string(term).strip("()")
    term = re.sub(r'^[A-Z]{1,4}\s*=\s*', '', term, flags=re.I).strip()
    term = term.strip('`\'"')
    if not term:
        return ""
    # Keep simple publication numbers as they are.
    if re.match(r"^[A-Z]{2}\d+[A-Z]?\d?$", term.upper()):
        return term.upper()
    # Quote phrases, leave one-word terms unquoted.
...
def make_or_group(terms, exact=False, max_terms=6):
    """
    Create the content inside one Google Patents field box.
    Correct pattern: TAC=(term1 OR "term two" OR term3)
    Not: TAC=((term1 OR term2) "domain")
    """
    cleaned = []
    for t in terms[:max_terms]:
        if exact:
            formatted = quote_gp_phrase(t)
        else:
            formatted = gp_term(t, exact=False)
            if " " in formatted and not formatted.startswith('"'):
                formatted = quote_gp_phrase(formatted)
        if formatted:
...
def make_field_query(field, terms, exact=False, max_terms=6):
    group = make_or_group(terms, exact=exact, max_terms=max_terms)
    return f"{field}=({group})" if group else ""


def make_and_field_query(field, term_groups):
    """
    Google Patents AND logic is most reliable as multiple field boxes:
    TAC=(core1 OR core2) TAC=(application1 OR application2)
    """
    boxes = []
    for terms in term_groups:
        ...
        if isinstance(terms, str):
            terms = [terms]
        q = make_field_query(field, terms, exact=False, max_terms=6)
        if q:
            boxes.append(q)
    return " ".join(boxes)


def make_free_text_query(term_groups):
    """Natural Google Patents query. Useful because Google also searches title/abstract/claims/description."""
    parts = []
    for terms in term_groups:
        if isinstance(terms, str):
            terms = [terms]
        group = make_or_group(terms, exact=False, max_terms=5)
        if group:
            if " OR " in group:
...
def split_boolean_query(query):
    """Turn AI-style exact AND/OR queries into term groups."""
    q = normalize_query_string(query)
    q = re.sub(r"\b(TAC|TI|AB|CL)\s*=\s*", "", q, flags=re.I)
    q = q.replace("`", "")
    ...
    # Split AND into separate Google Patents boxes; preserve OR inside each box.
    and_parts = re.split(r"\s+AND\s+", q, flags=re.I)
    groups = []
    for part in and_parts:
        ...
        for t in or_parts:
            ...
            if not t:
                continue
            # Remove field syntax remnants and outer quotes only; quote_gp_phrase will re-quote phrases.
            t = re.sub(r"\b(TAC|TI|AB|CL|CPC|IPC)\s*=\s*", "", t, flags=re.I).strip()
            t = t.strip("() ")
            if t:
                terms.append(t.strip('"'))
        if terms:
            groups.append(dedupe_keep_order(terms))
    return groups


def sanitize_ai_query_to_google_patents(query):
    """
    Convert AI-generated queries into reliable Google Patents syntax.
    Correct examples:
      TAC=("boron doped diamond" OR BDD) TAC=(sensor OR electrode)
      TAC=(PECVD OR "chemical vapor deposition") TAC=(diamond)
    """
    q = normalize_query_string(query)
    if not q:
        return ""

    # If the user pasted a publication number, search it directly.
    pub = find_patent_publication_number(q)
    if pub:
        return pub

    groups = split_boolean_query(q)
    if not groups:
        terms = extract_terms_from_query(q, max_terms=8)
        if not terms:
            return q
        groups = [terms]

    # Avoid over-narrowing: max 3 AND boxes, max 6 OR terms per box.
    groups = groups[:3]
    fielded = make_and_field_query("TAC", groups)
    return fielded or make_free_text_query(groups)


def make_domain_query(core_terms, domain):
    core_terms = core_terms[:5]
    domain_terms = split_prompt_terms(domain, max_terms=4) or extract_terms_from_query(domain, max_terms=4)
    if core_terms and domain_terms:
        return make_and_field_query("TAC", [core_terms, domain_terms])
    return ""


def build_patent_query_plan(ai_queries, keywords, domains, summary, value_proposition="", mode="balanced"):
    """
    Builds a safer Google Patents query plan.

    Main rule:
    - OR terms stay inside one field box: TAC=(term1 OR term2)
    - AND concepts become multiple field boxes: TAC=(core terms) TAC=(application terms)
    This mirrors how Google Patents advanced-search boxes work and avoids overly strict/invalid AI queries.
    """
    mode = mode or "balanced"
    plan = []

    def add(label, query, origin="auto"):
        query = normalize_query_string(query)
        if not query or len(query) < 3:
            return
        if len(query) > 320:
            query = query[:320].rsplit(" ", 1)[0]
        key = query.lower()
        if key not in {p["query"].lower() for p in plan}:
            plan.append({"label": label, "query": query, "origin": origin})

    ai_queries = [normalize_query_string(q) for q in (ai_queries or []) if normalize_query_string(q)]
    ...
    extracted_from_ai = []
    for q in ai_queries:
        extracted_from_ai.extend(extract_terms_from_query(q, max_terms=10))

    extracted_from_summary = extract_terms_from_query(summary + " " + value_proposition, max_terms=10)
    ...
    if mode == "broad":
        for t in core_terms[:8]:
            add(f"Single concept TAC: {t}", make_field_query("TAC", [t], exact=False, max_terms=1), "auto-relaxed")
...
def build_fallback_queries(query):
    """Fallbacks for one query if it returns suspiciously few patents."""
    q = normalize_query_string(query)
    groups = split_boolean_query(q)
    terms = extract_terms_from_query(q, max_terms=10)
    variants = [q]

    if groups:
        variants.append(make_and_field_query("TAC", groups[:3]))
        variants.append(make_free_text_query(groups[:3]))

    if terms:
        variants.append(make_field_query("TAC", terms, exact=False, max_terms=7))
        variants.append(make_field_query("AB", terms, exact=False, max_terms=6))
        variants.append(make_free_text_query([terms[:7]]))
        relaxed = q.replace('"', " ")
        relaxed = re.sub(r"\bAND\b", " ", relaxed, flags=re.I)
        relaxed = re.sub(r"\bOR\b", " ", relaxed, flags=re.I)
        relaxed = re.sub(r"\b(TAC|TI|AB|CL)\s*=\s*", " ", relaxed, flags=re.I)
...
if st.session_state.search_queries:
    st.markdown("---")
    st.markdown("### 🔍 Patent Query Builder — vyhledávací dotazy")
    ...
    with st.expander("Upravit AI dotazy", expanded=False):
        updated = []
    ...
    query_plan_preview = build_patent_query_plan(
        st.session_state.search_queries,
        st.session_state.tech_keywords_en,
        st.session_state.tech_domains,
...
if st.session_state.phase >= 2 and st.session_state.search_queries and not st.session_state.patents_raw:
    ...
    if st.button("🔎 Spustit patentovou rešerši + AI filtr", type="primary", disabled=not serpapi_key):
        ...
        if not query_plan:
            st.error("Nepodařilo se vytvořit žádné patentové dotazy. Uprav klíčová slova nebo prompt.")
            st.stop()
        ...
        if not all_patents:
            st.error("Nebyly nalezeny žádné patenty ani po rozšíření dotazů. Zkus režim Broad, ručně zjednoduš dotazy, nebo ověř SerpApi klíč.")
            with st.expander("Diagnostika dotazů", expanded=True):
                if query_audit:
                    st.dataframe(pd.DataFrame(query_audit), use_container_width=True, hide_index=True)
            progress.empty()
            st.stop()
...
if st.session_state.patents_raw:
    ...
    if st.session_state.query_audit:
        with st.expander("Diagnostika patentových dotazů", expanded=False):
            st.caption("Ukazuje, které AI/auto dotazy reálně něco našly. Když má dotaz nulu, aplikace ho v režimu Balanced/Broad automaticky zkouší uvolnit.")
            st.dataframe(pd.DataFrame(st.session_state.query_audit), use_container_width=True, hide_index=True)
